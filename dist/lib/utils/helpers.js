 ...
 */
const getRootDir = () => {
  let rootDir = isNodeModule() ? packageDir.split(path.sep).slice(0, splitPath.length - 2).join(path.sep) : packageDir;

  return rootDir;
};

const isNodeModule = () => {
  let parentDir = path.basename(splitPath.slice(0, splitPath.length - 1).join('/'));
